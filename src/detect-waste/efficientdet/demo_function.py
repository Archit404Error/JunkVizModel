...
def get_args_parser():
    parser = argparse.ArgumentParser("Test detr on one image")
    parser.add_argument(
        "--img",
        metavar="IMG",
...
def rescale_bboxes(out_bbox, size, resize):
    ...
    b = out_bbox * torch.tensor(
        [img_w / out_w, img_h / out_h, img_w / out_w, img_h / out_h],
        dtype=torch.float32,
    ).to(out_bbox.device)
    return b
...
def get_output(img, prob, boxes, classes=["Litter"], stat_text=None):
    ...
    if stat_text is not None:
        cv2.putText(
            img, stat_text, (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 10
        )
        cv2.putText(
            img, stat_text, (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 3
        )
    return img
...
def plot_results(pil_img, prob, boxes, classes=["Litter"], save_path=None, colors=None):
    ...
    def draw_text(
        ...
        cv2.rectangle(
            img, pos, (x + text_w, y + text_h + font_scale * 3), text_color_bg, -1
        )
        cv2.putText(
            img,
            text,
...
def main(im):
    ...
    imageOriginal, imageData, prob, boxes = plot_results(
        im, probas, bboxes_scaled.tolist(), args.classes, args.save
    )
